...
class MapModel
{
	current = new Leaf < Site.Model | null > ( null );
	zoom : Leaf.Number;
	center : Leaf < LatLong >;
	...
	hover = new Leaf < Site.Model | null > ( null );
	...
	updateCurrent( newItem : Site.Model | null, oldItem ? : Site.Model | null )
	{
...
namespace Site
{
	...
	export const list = sitepub.split( "\n" ).map( csv => new Model( csv ) );

	export const UI = ( site : Model, map : MapModel ) =>
	{
...
const MapFrame = ( model : MapModel ) =>
{
	...
	(
		...
		... Site.list.map( data => Site.UI( data, model ) )
	);
